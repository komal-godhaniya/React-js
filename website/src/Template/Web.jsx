import React from 'react'

export default function Web() {
  return (

<div>

    <header>
    <div class="logo"></div>
     <nav class="head">
       <center>
           <a href="">Home</a>
           <a href="">About</a>
           <a href="">Blog</a>
           <a href="">Pages</a>
           <a href="">Services</a>
           <a href="">Contact</a>
 
       </center>
     </nav>
 </header>

 <div class="section1">
  <img src="/images/banner.png" alt=""/>

  <div class="dog">
    <img src="/images/dog.png" alt=""/>
  </div>

  <div class="title">
     <h1>
       we care <br/>
     <span>Your Pets</span>
     </h1>
     
     <p>Lorem ipsum dolor sit amet, consectetur <br/>
       adipiscing elit, sed do eiusmod
     </p>

     <a href="" class="btn1">Contact Us</a>

   </div>
  
 </div>

 <div class="section2">

   <center>
     <div class="title2">
       <h1>Services for every dog</h1>
        <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor <br/>
          incididunt ut labore et dolore magna.</p>
     </div>
   </center>

   <div class="card1">
     <img src="/images/service_icon_bg_1.png" alt=""/>
     <img class="img2" src="/images/service_icon_1.png" alt=""/>
     <center>
       <h1>Pet Boarding</h1>
       <p>Lorem ipsum dolor sit amet,<br/>
          consectetur adipiscing elit, sed do <br/>
          eiusmod tempor incididunt ut</p>
     </center>
   </div>

   <div class="card2">
     <img src="/images/service_icon_bg_1.png" alt=""/>
     <img class="img2" src="/images/service_icon_2.png" alt=""/>
     <center>
       <h1>Healthy Meals</h1>
       <p>Lorem ipsum dolor sit amet,<br/>
         consectetur adipiscing elit, sed do <br/>
         eiusmod tempor incididunt ut</p>

     </center>

   </div>

   <div class="card3">
     <img src="/images/service_icon_bg_1.png" alt=""/>
     <img class="img2" src="/images/service_icon_3.png" alt=""/>
     <center>
       <h1>Pet Spa</h1>
       <p>Lorem ipsum dolor sit amet,<br/>
         consectetur adipiscing elit, sed do <br/>
         eiusmod tempor incididunt ut</p>

     </center>

   </div>

 </div>
 <div class="section3">
   <img src="/images/pet_care.png" alt=""/>
   <div class="tag1">
 <h1>
   We care your pet <br/>
   <span>As you care</span>
 </h1>
 <p>
   Lorem ipsum dolor sit , consectetur adipiscing elit, sed do <br/>
   iusmod tempor incididunt ut labore et dolore magna aliqua. <br/>
   Quis ipsum suspendisse ultrices gravida. Risus commodo <br/>
   viverra maecenas accumsan <br/>
 </p>

 <a href="" class="btn2">About Us</a>
   </div>
 </div>

 <div class="section4">
   <div class="tag2">
     <h1>
       we need Your <br/>
       <span>help Adopt Us</span>
     </h1>
     <p>
       Lorem ipsum dolor sit , consectetur adipiscing elit, sed do <br/>
       iusmod tempor incididunt ut labore et dolore magna aliqua. <br/>
       Quis ipsum suspendisse ultrices gravida. Risus commodo <br/>
       viverra maecenas accumsan <br/>
     </p>
   
     <a href="" class="btn2">Contact Us</a>
   </div>
   
   <div class="box1">
     <center>
       <img src="/images/dog 1.png" alt=""/>
        <h1>450</h1>
        <h2>Pets Available</h2>
     </center>
   </div>

   <div class="box2">
     <center>
       <img src="/images/dog 2.png" alt=""/>
        <h1>52+</h1>
        <h2>Pets Available</h2>
     </center>
   </div>

   <div class="box3">
     <center>
       <img src="/images/dog 3.png" alt=""/>
        <h1>10+</h1>
        <h2>Pets Available</h2>
     </center>
   </div>
 </div>

<div class="section5">
   <div class="pr1">
     <img src="/images/thought.png" alt=""/>
     <h1>John Wolker <br/>
      <span>Head of web design</span>
     </h1>

     <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do <br/>
       eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim <br/>
       d minim veniam, quis nostrud exerci.</p>
   </div>
   
 </div>
<div class="section6">

 <center>
   <div class="title3">
     <h1>Our Team</h1>
      <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor <br/>
        incididunt ut labore et dolore magna.
      </p>
   </div>
 </center>

<img src="/images/manager1.png" alt=""/>
<img class="img2" src="/images/manager2.png" alt=""/>
<img class="img3" src="/images/manager3.png" alt=""/>

<div class="text1">
   <center>
 
    <h1>Rala Emaia </h1>
    <h2>Senior Director</h2>
 
   </center>
</div>

 <div class="text2">
   <center>
 
    <h1>jhon Smith</h1>
    <h2>Senior Director</h2>
 
   </center>
</div>

  <div class="text3">
   <center>
 
    <h1>Rala Emaia </h1>
    <h2>Senior Director</h2>
 
   </center>
</div>

</div> 

<div class="section7">
 
 <img src="/images/contact_bg.png" alt=""/>

 <div class="heading">

  <h1>Why go with Anipat?</h1>

  <p>Because we know that even the best technology is only as <br/>
    good as the people behind it. 24/7 tech support.
  </p>
 
  <a href="" class="btn4">Contact Us</a>
<h3>or +880 9856 723</h3>
 </div>
</div>

<div class="section8">

 <h1>
   Contact us
 </h1>
 <h2>
   +555 0000 565 <br/>
   Demomail@gmail.Com<br/>  
    700, Green Lane, New York, USA
 </h2>

   <h3>
  Our Servces
 </h3>
 <h4>
   pet insurance<br/>
   pet Adoption<br/> 
   Dog insurance<br/>
   Dog Adoption<br/> 
   Cat Adoption<br/> 
 </h4>

 <h5>
 Quick Link
 </h5>

 <h6>
   
 About Us <br/>
 Privacy Policy<br/>
 Terms of Service<br/>
 Login info<br/>
 Knowledge Base<br/>

 </h6>
 <img src="/images/logo (1).png" alt=""/>
 <p>
 239 E 5th St, New York NY 10003,<br/>
 USA
 </p>
 <i class="ri-facebook-circle-line fb"></i>
 <i class="ri-pinterest-line pr"></i>
 <i class="ri-at-line a"></i>



</div>
 





</div>

  )
}
